...
const client = await KineticSdk.setup({ environment, index })
...
```js
// Kinetic
await client.createAccount({ owner: keypair })
...
```js
// Kinetic
await client.requestAirdrop({ account, amount })
...
```js
// Kinetic
await client.makeTransfer({ amount, destination, owner, type })
...
await client.makeTransferBatch({ owner, destinations })
