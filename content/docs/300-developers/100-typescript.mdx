...
Once you're ready to code, have a quick look at our [Developer Best Practices](/essentials/best-practices/) where we cover some useful topics that you'll want to keep in mind as you build out you application.
...
- For methods that request a token be transferred, `amount` is the amount of the token, not quarks.
...
// Kinetic
import { KineticSdk } from '@kin-kinetic/sdk'
const client = await KineticSdk.setup({ environment, index, endpoint })

// Agora
import { Client } from '@kinecosystem/kin-sdk-v2'
const client = new Client(env, { appIndex })
```
...
```js
// Kinetic
await client.createAccount({ owner: keypair, commitment })
...
```js
// Kinetic
await client.requestAirdrop({ account, amount, commitment })

// Agora
await client.requestAirdrop(tokenAccountPublicKey, quarks)
```
...
```js
// Kinetic
await client.makeTransfer({ amount, destination, owner, commitment, type, reference, referenceType, senderCreate })

// Agora
await client.submitPayment({ quarks, destination, sender, type })
...
await client.makeTransferBatch({ owner, destinations, commitment, type })

// Agora
const earns = [{ destination, quarks }]
await client.submitEarnBatch({ sender, earns })
```
